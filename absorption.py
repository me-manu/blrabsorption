...
class BLR(object):
    def __init__(self, Ldisk, z, lumiHbeta = 4.2e43, radiusHbeta = 0., lumi5100 = 0., M_BH = 1e9):
        ...
        self._Ldisk = Ldisk
        self._M_BH = M_BH
        self._blr_lines = Table.read(path.join(path.dirname(blrabsorption.__file__), 'blr_line_pars.fits'))
        self._z = z
    ...
    @property
    def radiusHbeta(self):
        return self._radiusHbeta
    ...
    def energy_density(self, Rcm, 
        ...
        ----------
        Rcm: float or `~numpy.ndarray`
            distance of gamma-ray emitting region in cm, n-dimensional
        chi: float, optional
            factor for geometry, default: 0.01
        ...
        if geometry == 'shell':
            ...
            xsq = self._Rli**2. + Rcm**2.
            result = self._xili / xsq
        elif geometry == 'extended': 
            result = self._xili * chi
            result /= self._Rdt_in ** 2.
            #R = np.logspace(np.log10(self._Rdt_in),
                #np.log10(self._Rdt_out), 100)
            #result *= simps((R / self._Rdt_in)**(-self._zeta) / (R**2. + Rcm**2.), R)
            #if self._zeta == 1. : 
            #    result *= (np.log((Rcm**2. +  self._Rdt_in**2.) / (Rcm**2. + self._Rdt_out**2.)) + \
            #                2. * np.log(self._Rdt_in / self._Rdt_out)) / 2. / Rcm**2.
            #    result *= self._Rdt_in
            #else: 
            #    def primitive(R,r,zeta):
            #        return R**(1. - zeta) / (zeta - 1.) / r**2. * \
            #            hyp2f1(1., 0.5*(1. - zeta), 0.5*(3. - zeta), -R**2. / r**2.)
            #    result *= (primitive(self._Rdt_out, Rcm, self._zeta) - 
            #                primitive(self._Rdt_in, Rcm, self._zeta))
            #    result *= self._Rdt_in ** zeta
            #result /= self._Rdt_eff

        # only one factor of 4 pi, since we integrated over solid angle for the isotropic field
        result *= self._Ldisk / (4. * np.pi) / c.c.cgs.value
        return result
...
class DustTorus(BLR):
    def __init__(self, Ldisk, z, 
        ...
        self.Rli = np.array([3.5e18 * np.sqrt(Ldisk / 1e45) * (Tdust / 1e3) ** -2.6]) # sublimation radius
...
class AccretionDisk(BLR):
    ...
    def __stilde_disk(self, eps, Rtilde, ltilde):
        return 0.5 * self.eps0(Rtilde) * eps * (1. + self.z) * (1. - self.__mu(Rtilde, ltilde))
